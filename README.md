<p align="center">
  <img src="public/logo-dark.svg" alt="Laravel Page Builder" width="350">
</p>

<p align="center">
<a href="https://packagist.org/packages/coderstm/laravel-page-builder"><img src="https://img.shields.io/packagist/v/coderstm/laravel-page-builder" alt="Latest Stable Version"></a>
<a href="https://packagist.org/packages/coderstm/laravel-page-builder"><img src="https://img.shields.io/packagist/dt/coderstm/laravel-page-builder" alt="Total Downloads"></a>
<a href="https://packagist.org/packages/coderstm/laravel-page-builder"><img src="https://img.shields.io/packagist/l/coderstm/laravel-page-builder" alt="License"></a>
<a href="https://github.com/coders-tm/laravel-page-builder/actions"><img src="https://github.com/coders-tm/laravel-page-builder/workflows/tests/badge.svg" alt="Build Status"></a>
</p>

<p align="center">
A modern page builder for Laravel — build dynamic pages with layouts, sections, and JSON rendering. Includes a visual editor, layout system, reusable sections, and multi-theme support.
</p>

<p align="center">
  <a href="https://coders-tm.github.io/laravel-page-builder/"><strong>Documentation</strong></a>
</p>

<p align="center">
  <img src="art/intro.gif" alt="Laravel Page Builder Intro" />
</p>

---

## Requirements

- PHP 8.2+
- Laravel 11.x, 12.x, or 13.x

## Installation

```bash
composer require coderstm/laravel-page-builder
```

```bash
php artisan pagebuilder:install
```

See the [full documentation](https://coders-tm.github.io/laravel-page-builder/) for configuration, sections, blocks, templates, themes, and more.

---

## License

This package is released under a **Non-Commercial Open Source License**. Contact [dipak@coderstm.com](mailto:dipak@coderstm.com) for commercial licensing.

See [LICENSE.md](LICENSE.md) for details.
