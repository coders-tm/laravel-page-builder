...
class PageService
{
    ...
    public function render(string $slug, array $meta = [], bool $editor = false): mixed
    {
        ...
        // ── 1. Page view (Custom or Cached) ───────────────────────────────
        // When a page view exists, we load it. If it was generated by the builder,
        // we only skip it if we are explicitly in editor mode.
        if (View::exists("pages.{$slug}")) {
            $isGenerated = $this->pageStorage->isGeneratedView($slug);

            if (! $isGenerated || (! $editor && ! PageBuilder::editor())) {
        ...
        // ── 3. Page builder JSON ──────────────────────────────────────────
        if ($stored !== null) {
            return $this->renderWithCache($slug, $page, $dbPage, $meta);
        }
    ...
    /**
     * Render a page with view caching support.
     */
    protected function renderWithCache(string $slug, PageData $page, mixed $dbPage, array $meta): mixed
    {
        $html = $this->pageRenderer->renderPage($page);

        // Save to view cache if enabled
        if (config('pagebuilder.cache.enabled', false)) {
            $this->saveViewCache($slug, $page, $html);
        }
    ...
    /**
     * Save the rendered HTML as a builder-generated Blade view.
     */
    protected function saveViewCache(string $slug, PageData $page, string $html): void
    {
        $source = sprintf(
            "@extends('%s')\n\n@section('content')\n%s\n@endsection",
            $page->layoutView(),
        ...
        $this->pageStorage->saveView($slug, $source);
    }
