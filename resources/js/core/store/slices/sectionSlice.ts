...
export interface SectionSlice {
  updateSectionSettings: (sectionId: string, settingsPatch: Record<string, any>) => void
  addSection: (
    type: string,
    schema: any,
...
> = (set, get) => ({
  ...
  removeSection: (sectionId) =>
    set((state) => {
      ...
      state.currentPage.order = state.currentPage.order.filter((id) => id !== sectionId)
  ...
  duplicateSection: (sectionId) => {
    ...
    set((state) => {
      ...
      ;(original.order || []).forEach((bid: string) => {
        ...
        newBlocks[newBid] = JSON.parse(JSON.stringify(original.blocks?.[bid] || {}))
  ...
  reorderSections: (fromIndex, toIndex) =>
    set((state) => {
      ...
      const pageIds = flatOrder.filter((id) => !(sections[id] as any)?.layout)
