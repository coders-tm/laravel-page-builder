...
export interface BlockSlice {
  ...
  duplicateBlock: (sectionId: string, blockId: string, parentPath?: string[]) => string
  reorderBlocks: (sectionId: string, newBlockOrder: string[], parentPath?: string[]) => void
  toggleBlockDisabled: (sectionId: string, blockId: string, parentPath?: string[]) => void
  moveBlock: (
    ...
    set((state) => {
      ...
      if (parentPath.length > 0) {
        ...
        if (parent && parent.blocks) {
          ...
          parent.order = (parent.order || []).filter((id: string) => id !== blockId)
        }
      } else {
        delete section.blocks[blockId]
        section.order = (section.order || []).filter((id: string) => id !== blockId)
      }
  ...
  toggleBlockDisabled: (sectionId, blockId, parentPath = []) =>
    set((state) => {
      ...
      if (parentPath.length > 0) {
        const parent = getNestedBlock(section.blocks, parentPath)
        if (parent && parent.blocks?.[blockId]) {
          parent.blocks[blockId].disabled = !parent.blocks[blockId].disabled
        }
      } else {
        if (section.blocks?.[blockId]) {
  ...
  moveBlock: (fromSectionId, toSectionId, blockId, fromPath, toPath, toIndex) =>
    set((state) => {
      ...
      if (fromPath.length > 0) {
        ...
        if (parent && parent.blocks?.[blockId]) {
          blockData = JSON.parse(JSON.stringify(parent.blocks[blockId]))
          delete parent.blocks[blockId]
          parent.order = (parent.order || []).filter((id: any) => id !== blockId)
        }
      } else {
        if (fromSection.blocks?.[blockId]) {
          blockData = JSON.parse(JSON.stringify(fromSection.blocks[blockId]))
          delete fromSection.blocks[blockId]
          fromSection.order = (fromSection.order || []).filter((id: any) => id !== blockId)
        }
      }
