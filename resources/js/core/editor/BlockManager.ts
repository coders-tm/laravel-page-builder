...
export class BlockManager {
  ...
  private isLocalBlockEntry(entry: { type: string; [key: string]: any }): boolean {
  ...
  private resolveParentRawBlocks(
    ...
    for (const blockId of parentPath) {
      ...
      const blockSchema = this.resolveSchema(block.type, parentRawBlocks, state.blocks)
  ...
  getAddableTypes(sectionId: string, parentPath: string[] = []): BlockSchema[] {
    ...
    return raw
      ...
      .map((entry) => {
        ...
        return themeBlocks[entry.type]?.schema ?? (entry as unknown as BlockSchema)
      })
  }
  ...
  duplicate(sectionId: string, blockId: string, parentPath: string[] = []): string {
    const newId = useStore.getState().duplicateBlock(sectionId, blockId, parentPath)
  ...
  reorder(sectionId: string, newOrder: string[], parentPath: string[] = []): void {
  ...
  updateSettings(
    ...
    useStore.getState().updateBlockSettings(sectionId, blockId, values, parentPath)
  ...
  toggleDisabled(sectionId: string, blockId: string, parentPath: string[] = []): void {
  ...
  move(
    ...
    useStore.getState().moveBlock(fromSectionId, toSectionId, blockId, fromPath, toPath, toIndex)
  ...
  rename(sectionId: string, blockId: string, name: string, parentPath: string[] = []): void {
    useStore.getState().renameBlockInstance(sectionId, blockId, name, parentPath)
