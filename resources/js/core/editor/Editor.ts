...
export class Editor {
  ...
  constructor(config: Partial<EditorConfig> & Partial<PageBuilderConfig> = {}) {
    ...
    this.navigation = new NavigationManager(this.events, this.selection, this.layout)
    this.bootstrap = new BootstrapManager(this.events, this.pages, this.history, this.navigation)
  ...
  private _wirePreviewEvents(): void {
    ...
    this.events.on(
      "section:toggled",
      ({ sectionId, disabled }: { sectionId: string; disabled: boolean }) => {
    ...
    this.events.on(
      ...
      ({
        ...
        const parentBlockId =
          parentPath && parentPath.length > 0 ? parentPath[parentPath.length - 1] : null
    ...
    this.events.on("theme:settings-reset", ({ cssVars }: { cssVars: Record<string, string> }) => {
  ...
  addBlock(
    ...
    return this.blocks.add(sectionId, type, defaults, afterBlockId, parentPath)
  }
  ...
  renameBlock(sectionId: string, blockId: string, name: string, parentPath: string[] = []): void {
  ...
  removeBlock(sectionId: string, blockId: string, parentPath: string[] = []): void {
  ...
  addBlockFromPreview(payload: {
    ...
    const targetOrder: string[] = parentNode?.order || Object.keys(parentNode?.blocks || {})
    ...
    if (blockTypesWithLimits.length === 1 && enabledTypes.length === 1) {
      ...
      const blockId = this.addBlock(sectionId, blockType.type, defaults, afterBlockId, parentPath)
      this.selectBlock(sectionId, [...parentPath, blockId])
      return
    }

    this.layout.openAddBlockModal(blockTypesWithLimits, sectionId, parentPath, afterBlockId)
  }
